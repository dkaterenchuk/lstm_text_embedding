...
def get_models(vocab_size=10000, input_length=64, word_dim=64, sentence_dim=256):
    """
    ...
    """
    # encoder
    input_layer = Input(shape=(input_length,), batch_shape=(16, input_length))
    embedding_layer = Embedding(vocab_size, word_dim)(input_layer)
    lstm_encoder_layer = LSTM(sentence_dim, return_sequences=False, activation="tanh")(embedding_layer)

    # decoder
    repeat_layer = RepeatVector(input_length)(lstm_encoder_layer)
    lstm_decoder_layer = LSTM(sentence_dim, return_sequences=True, activation="tanh")(repeat_layer)
    dense_layer = TimeDistributed(Dense(1, activation="relu"))(lstm_decoder_layer)
...
def train_model(lstm_autoencoder, lstm_encoder, sequence_generator):
    """
    ...
    """
    lstm_autoencoder.fit(sequence_generator, sequence_generator, batch_size=16)
